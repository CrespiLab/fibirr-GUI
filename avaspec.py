...
if 'linux' in sys.platform: # Linux will have 'linux' or 'linux2'
    ...
    if (ctypes.sizeof(ctypes.c_voidp) == 8): # 64 bit
        ...
        lib = ctypes.WinDLL("./avantes/avaspecx64.dll")
        func = ctypes.WINFUNCTYPE
    else:
        WM_MEAS_READY = 0x0401
        # lib = ctypes.WinDLL("./avaspec.dll")
        lib = ctypes.WinDLL("./avantes/avaspec.dll")
        func = ctypes.WINFUNCTYPE
...
def AVS_SuppressStrayLight(handle, a_MultiFactor, a_pSrcSpectrum):
    ...
    Returns the stray light corrected pixel values of a dark corrected measurement. Can be 
    called by the application if a new spectrum is received (AVS_GetScopeData()) and a dark 
    spectrum has been subtracted. The Library does not check the allocated buffer size! 

    :param handle: AvsHandle returned by AVS_Activate or others
    :param a_MultiFactor: Multiplication factor in stray light algorithm 
        The range allowed for this parameter is 0.0 – 4.0, where 0.0 means no correction and a higher 
